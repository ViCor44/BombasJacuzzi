...
#define MEASURE_INTERVAL 60000 //3 minute intervals between measurements (in ms)
...
void ListHours(EthernetClient client) {
    ...
    if (client) {        
        ...
        while (client.connected()) {
            if (client.available()) {
                ...
                else if (strstr(clientline, "GET /horas")) {
                    // send a standard http response header
                    HtmlHeaderOK(client);
                    // print all the data files, use a helper to keep it clean
                    client.println("<h1 style=\"font-family:arial;\">Graficos de Horas Trabalhadas</h1>");
                    client.println("<h2>Ver graficos da semana(dd-mm-aa):</h2>");
                    ListHours(client);
                }
  ...
  if ((millis() % lastIntervalTime) >= MEASURE_INTERVAL){ //Is it time for a new measurement?
    ...
    if (rawTime != 39){                         //If that worked, and we have a real time
      ...
      if (dataFile) {
        ...
        dataFile.print(digitalRead(counterpin1));
        dataFile.print(",");
        dataFile.print(digitalRead(counterpin2));
        dataFile.println();
        dataFile.close();
